...
async function loadReward(slug: string): Promise<PublicContentPublicRecord | null> {
  try {
    return await getPublishedContentBySlug(slug, "reward");
  } catch (error) {
    const reason = error instanceof Error ? error.message : "unknown";
    console.warn("[PUBLIC_REWARD_LOOKUP_SKIPPED]", reason.slice(0, 180));
    return null;
  }
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const content = await loadReward(slug);
  if (!content) return {};
  return {
...
export default async function PublicRewardPage({ params }: PageProps) {
  const { slug } = await params;
  const content = await loadReward(slug);
  if (!content) notFound();
