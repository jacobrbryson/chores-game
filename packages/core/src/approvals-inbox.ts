...
// itself goes through `PATCH /api/chores/{id}`.
//
// This lives in @packages/core because the web /approvals page and the mobile
// Approvals screen render the same queue; the grouping, ordering, and the
// needs-coins split must not drift between the two apps.
import {
  choreNeedsCoinAssignmentPrompt,
  shouldHideChoreCoinValue,
} from "./chore-coin-assignment";
...
// Even split of a chore's coin value across its assignees — the default the
// coin-review step starts from before a parent adjusts individual payouts.
export function defaultCoinsByAssignee(chore: ApprovalChore): Record<string, number> {
...
// Quick-pick coin amounts offered in the coin-review step.
export const APPROVAL_COIN_QUICK_VALUES = [5, 10, 15, 20];
