...
- Smart Ghost Chores update (2026-06-02):
  ...
  - Exclusions: a suggestion is never shown if the viewer already has a chore with the same title that is currently open OR set up as a *repeating* chore (any status — recurring chores recur regardless of current status), nor if the viewer has already dismissed/requested it (deduplicated by normalized title key).
